...
function ExampleGroup({ lines }: { lines: ExampleLine[] }) {
  const results = evaluate(lines.map((l) => l.input));
  return (
    <div className="flex max-w-xl font-mono text-xs p-2 rounded-lg border-border border">
      ...
      <div className="min-w-0 flex-1 space-y-0.5 text-right">
        {lines.map((line, i) => {
          ...
          return (
            <div key={i} className={`truncate ${isAssignment ? "text-accent-dim" : "text-accent"}`}>
              {display ?? "\u00A0"}
            </div>
          );
        })}
  ...
  return (
    <>
      ...
      <Modal
        open={open}
        onClose={close}
        overlayTestId="help-overlay"
        cardTestId="help-modal"
        cardClassName="mx-4 max-h-[80vh] w-full max-w-lg overflow-y-auto rounded-lg border border-border bg-surface p-6 shadow-lg sm:max-w-4xl"
      >
        <div className="mb-5 flex items-center justify-between">
          <h2 className="font-sans text-base font-semibold text-foreground">Help</h2>
          <button
        ...
        <div className="space-y-5">
          ...
          <Section title="Comments">
            ...
            <div className="mt-1 space-y-0.5">
              <ExampleGroup lines={[{ input: "// monthly budget" }, { input: "# tax rate" }]} />
            </div>
          </Section>

          <Section title="Variables">
            <p>Assign values to variables and reuse them on later lines.</p>
            <div className="mt-1 space-y-0.5">
              <ExampleGroup
                lines={[{ input: "price = 49.99" }, { input: "qty = 3" }, { input: "price * qty" }]}
              />
            </div>
          </Section>
          ...
          <Section title="Unit Conversion">
            <p>
              Convert between units with <code className="font-mono text-foreground">to</code> or{" "}
              <code className="font-mono text-foreground">as</code>. Use{" "}
              <code className="font-mono text-foreground">%</code> to format as percent.
            </p>
            <div className="mt-1 space-y-0.5">
              <ExampleGroup
                lines={[
          ...
          <Section title="Currencies">
            <p>
              Currencies work as units. Convert with{" "}
              <code className="font-mono text-foreground">to</code> or{" "}
              <code className="font-mono text-foreground">as</code>. Live rates refresh daily — see
              the coin button in the header.
            </p>
            <div className="mt-1 space-y-0.5">
              <ExampleGroup
                lines={[
                  { input: "100 USD to EUR" },
                  { input: "salary = 65000 USD" },
                  { input: "salary as NOK" },
                  { input: "100 USD + 50 EUR" },
                ]}
              />
            </div>
          </Section>
          ...
          <Section title="Built-in References">
            <p>Special variables that update automatically as you type.</p>
            <div className="mt-1 space-y-0.5">
              <ExampleGroup
                lines={[
                  {
          ...
          <Section title="Custom Functions">
            <p>Assign an expression with unknowns to create a function.</p>
            <div className="mt-1 space-y-0.5">
              <ExampleGroup
                lines={[
                  { input: "f = x^2 + 1" },
          ...
          <Section title="Derivatives">
            <p>
              Take the derivative with <code className="font-mono text-foreground">derivate</code>,{" "}
              <code className="font-mono text-foreground">derive</code>, or{" "}
              <code className="font-mono text-foreground">derivative</code>. The variable is
          ...
          <Section title="Integrals" tag={<Tag>Beta</Tag>}>
            <p>
              ...
              <code className="font-mono text-foreground">integral</code>, or{" "}
              <code className="font-mono text-foreground">antiderivative</code>. The variable is
              inferred automatically.
            </p>
            <div className="mt-1 space-y-0.5">
              <ExampleGroup
